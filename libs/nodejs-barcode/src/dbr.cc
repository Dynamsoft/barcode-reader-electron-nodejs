...
void DecodeFileSync(const FunctionCallbackInfo<Value>& args) {
...
void DecodeFileAsync(const FunctionCallbackInfo<Value>& args) 
{
	...
	BarcodeWorker *worker = new BarcodeWorker;	// create a worker
	worker->request.data = worker;	// put the request data
	strcpy(worker->filename, pFileName);	// copy the filename
	worker->callback.Reset(isolate, cb);	// Reset the state of callback
	worker->iFormat = iFormat;	
	worker->pResults = NULL;	// Set initial state of result to NULL
	worker->buffer = NULL;	// Clear buffer
